...
#
# VENDORED + Py3-PATCHED for EvoLiEZ (s10 alchemical RBFE tier). AmberTools25's
# stock softcore_setup.py is a Py2-era script that crashes on modern prmtops.
# Patches (search "PY3 PATCH"), all in the prmtop parser/writer, logic untouched:
#   1. load_prmtop: `line[0] is not "%"` -> `!= "%"` (Py3 string identity).
#   2. load_prmtop: skip all-whitespace tokens (space-padded final data lines
#      gave blank 8-char tokens -> int()/float() ValueError).
#   3. load_prmtop: skip `%COMMENT` lines between %FLAG/%FORMAT (ff19SB CMAP).
#   4. parser/writer: handle 'F' (fixed-point float) format (CMAP_PARAMETER) as
#      float, alongside 'E'/'I'/'a'.
# Pure stdlib; runs under any python3. Validated: GLY->ALA hybrid topology +
# 3-stage crgmask/scmask emitted (scripts/smoke_amber_softcore.py).
import sys
import re
import os
import subprocess
...
class AmberSystem:
    ...
    def load_prmtop(self, filename):
        ...
        block_name_re = re.compile(r'%FLAG\s+(\w+)')
        self.format_re = re.compile(r'%FORMAT\((\d+)(.)([\d\.]+)')
        ...
        while True:
            ...
            line = fp.readline()
            while line.startswith("%COMMENT"):   # PY3/modern-prmtop PATCH: newer
                line = fp.readline()              # prmtops put %COMMENT between
                                                 # %FLAG and %FORMAT (e.g. CMAP)
            ...
            line = fp.readline()
            if line == "": break
            while line[0] != "%":   # PY3 PATCH: was `is not` (identity, unreliable)
            ...
            # PY3 PATCH: skip all-whitespace tokens. The slicer above walks the
            # full format width, so a space-PADDED final data line yields blank
            # 8-char tokens -> int('        ') / float('        ') crashed on the
            # modern AmberTools prmtop. Real values survive (.strip() keeps them).
            if token_type == "I":
                self.blocks[name] = [int(x) for x in self.blocks[name] if x.strip()]
            elif token_type in ("E", "F"):   # PY3 PATCH: 'F' (CMAP fixed-point) too
                self.blocks[name] = [float(x) for x in self.blocks[name] if x.strip()]
    ...
    def save_prmtop(self, filename):
        ...
        type_table = {'I': 'd', 'E': 'E', 'a': 's', 'F': 'f'}  # PY3 PATCH: 'F' float (CMAP)
